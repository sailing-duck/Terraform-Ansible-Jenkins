...
resource "aws_instance" "mtc_main" {
  ...
  # user_data              = templatefile("./main-userdata.tpl", { new_hostname = "${random_id.mtc_node_id[count.index].dec}" })
  root_block_device {
  ...
  # not recommanded in the most of times
  # terraform will not consider provisioner as a change
  # we need to taint this instance and re-apply
  # && aws ec2 wait instance-status-ok --instance-ids ${self.id} --region ap-southeast-2
